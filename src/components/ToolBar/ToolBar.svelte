<script>				
	...
	function setPosition(node){
		...
		let rect = elm.parentNode.getClientRects()
		let posY = rect[0].top; 
...
<div use:setPosition in:fly class="flex absolute -mt-6 shadow absolute bg-white z-20">	
	<div class="rounded flex items-center shadow-lg border border-gray-200  text-gray-700">
			...
			<div class="px-2 cursor-pointer { e_classes.bold ? 'text-blue-600':''} font-medium hover:bg-gray-200 py-1" on:mousedown={() => toggle(STYLE.BOLD)}>
				B
			</div>
			<div class="px-3 cursor-pointer { e_classes.italic ? 'text-blue-600':''} italic hover:bg-gray-200 py-1" on:mousedown={() => toggle(STYLE.ITALIC)}>
				i
			</div>
			<div class="px-2 cursor-pointer { e_classes.underline ? 'text-blue-600':''} underline hover:bg-gray-200 py-1" on:mousedown={() => toggle(STYLE.UNDERLINE)}>
				U
			</div>
			<div class="px-2 cursor-pointer { e_classes.linethrough ? 'text-blue-600':''} line-through hover:bg-gray-200 py-1" on:mousedown={() => toggle(STYLE.LINETHROUGH)}>
				S
			</div>
			<div class="px-2 cursor-pointer { e_classes.code ? 'text-blue-600':''} line-through hover:bg-gray-200 py-2" on:mousedown={() => toggle(STYLE.CODE)}>
				<CodeIcon />
			</div>
			<div class="{ e_classes.link ? 'text-blue-600':''} cursor-pointer hover:bg-gray-200 text-sm border-r">
				<LinkInput setLink={setClass} on:close={close} {href} />
			</div>
			
			<div class="pl-1 cursor-pointer hover:bg-gray-200 py-1 ">
				<ColorPicker {setClass} klass={classes} />
			</div>
			<div class="px-1 cursor-pointer hover:bg-gray-200 border-r h-full flex items-center">
				<ColorPicker txt="bg" {setClass} klass={classes} />
			</div>
		
			<div class="px-2 { e_classes.justify ? 'text-blue-600':'text-gray-700'} cursor-pointer hover:bg-gray-200 py-1 h-full flex items-center" on:mousedown={() => toggleG(STYLE.JUSTIFY)}>
				<JustifyIcon />
			</div>
			
			<div class="px-2 { e_classes.left ? 'text-blue-600':'text-gray-700'} cursor-pointer hover:bg-gray-200 py-1 h-full flex items-center"  on:mousedown={() => toggleG(STYLE.LEFT)}>
				<LeftIcon />
			</div>
			
			<div class="px-2 { e_classes.center ? 'text-blue-600':'text-gray-700'} cursor-pointer hover:bg-gray-200 py-1 h-full flex items-center" on:mousedown={() => toggleG(STYLE.CENTER)}>
				<CenterIcon />
			</div>
			
			<div class="px-2 { e_classes.right ? 'text-blue-600':'text-gray-700'} cursor-pointer hover:bg-gray-200 py-1 h-full flex items-center border-r"  on:mousedown={() => toggleG(STYLE.RIGHT)}>
				<RightIcon />
			</div>
		
			<div class="cursor-pointer hover:bg-gray-200 h-full flex items-center">
